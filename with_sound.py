...
TARGET_CLASSES = [0, 2, 5]  # person and car
CLASS_NAMES = {0: 'person', 2: 'car',  5: 'bus'}

# Global flag for beeping control
is_beeping = False
beep_thread = None
...
def beep_sound():
    ...
    while is_beeping:
        ...
        # If using pygame instead of winsound, use these lines:
...
normal_bbox_color = (255, 0, 255)  # Regular pink/magenta color
warning_bbox_color = (0, 0, 255)  # Red color for warning
        ...
        if results[0].boxes.id is not None:
            ...
            for box, track_id, class_id in zip(xyxys, track_ids, class_ids):
                if class_id in TARGET_CLASSES:
                    ...
                    # Choose box color based on distance
                    bbox_color = warning_bbox_color if distance <= WARNING_DISTANCE else normal_bbox_color

                    # Create label with class name and tracking ID
                    label = f"{CLASS_NAMES[class_id]}-{track_id}"

                    # Draw box and label with the chosen color
                    annotator.box_label(box, label=label, color=bbox_color)
                    annotator.visioneye(box, center_point)

                    # Add distance text
                    text_size, _ = cv2.getTextSize(f"Distance: {distance:.2f} m", cv2.FONT_HERSHEY_SIMPLEX, 1.2, 3)
                    cv2.rectangle(im0, (x1, y1 - text_size[1] - 10), (x1 + text_size[0] + 10, y1), txt_background, -1)
                    cv2.putText(im0, f"Distance: {distance:.2f} m", (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                                txt_color, 3)

        # Update warning circle and beeping
